...
void ChatDialog::processBlockReply(QVariantMap msg){
	...
	qDebug()<<"process bloock reply from " + seeder;
	...
	if(blocksLeft.contains(blockNum)){
		...
		if (currentBlockCount >= 100) {
			...
			if (blockNum > currentBlockCount)
				qDebug() << "WRONG WRONG WRONG";
			int segment = (100+currentBlockCount-1)/currentBlockCount;
			qDebug() << "segment size = " + QString::number(segment);

			for (int i = segment*blockNum; (i < segment*(blockNum+1)) && (i < 100); i++) {
				visual->blockPos[i] = color;
			}
			visual->update();
			visual->show();
			QCoreApplication::processEvents();
